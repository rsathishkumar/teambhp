...
var Lightbox = {	
	...
	initialize: function(options) {
		...
		var objHighRes = document.createElement('a');
		objHighRes.setAttribute('id',this.getID('highRes'));
		objHighRes.setAttribute('target','_blank');
		objHighRes.innerHTML = this.options.strings.highRes;
		objDetailsNav.appendChild(objHighRes);
		...
		objOuterImageContainer.appendChild(objClose);
	...
	changeImage: function(imageNum){	
		...
		imgPreloader.onload=function(){
			...
			var highResSrc = imgPreloader.src.substring(0, imgPreloader.src.indexOf('styles/')+7);
			highResSrc += 'check_high_res';
			highResSrc += imgPreloader.src.substring(imgPreloader.src.indexOf('/public'));
			$(Lightbox.getID('highRes')).setAttribute("href", highResSrc);
...
// -----------------------------------------------------------------------------------
Event.observe(window,'load',function(){ Lightbox.initialize({autoPlay:false}); });
/*if(document.getElementById("allPhotos")!=null)
	{
		if(document.getElementById("allPhotos").length>0)
			{
				Event.observe(window,'load',function(){ Lightbox.initialize({autoPlay:false}); });
			}
	}
$('startShow').observe('click', function(){
	Lightbox.start($(this));
	alert('start');
});
$('allbox').observe('click', function(){
