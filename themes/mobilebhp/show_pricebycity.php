<?php
		...
		if($_POST['action']=='show')
					...
					while($data_entity=@mysqli_fetch_array($sql_vrnts))
						...
						if($entity_id!='')
							{
						$sql_p=@mysqli_query("select * from team_bhp_variant_price where city='".$_POST['city']."' and nid in(".@substr($entity_id,0,-1).") group by nid");
						...
						if($entity_id!='')
							{
							$totalamountsql=@mysqli_fetch_array(mysqli_query("select ex_showroom_price from team_bhp_variant_price where city='".$_POST['city']."' and nid in (".@substr($entity_id,0,-1).") order by nid"));
							...
							<div class="emiCalcBox roundAll5 ">
									...
									<div class="emiTable roundAll5">
										<table class="priceVariant emi">
											...
											<tbody>
											<?php
										$emisql=@mysqli_query("select * from team_bhp_variant_price where city='".$_POST['city']."' and nid in (".@substr($entity_id,0,-1).")");
												$cnt_emi=0;
