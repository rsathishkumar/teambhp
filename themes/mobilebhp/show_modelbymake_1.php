...
//		print_r($_POST);
			...
			if($_POST['makeid']==0)
			{
				...
				FROM node,field_data_field_make_home_page WHERE field_data_field_make_home_page.entity_id = node.nid  AND node.status =1 AND node.nid not in (".@substr($mid,0,-1).") order by rand()");
...
<!--					<img src="/sites/default/files/styles/check_mobile_thumb/public/--><?php //echo str_replace("public://","",$sql_img['uri']);?><!--" alt="--><?php //echo $data_model['title'];?><!--" />-->
<!--					<b>-->
...
<!--		</div>-->
		...
		FROM node,field_data_field_make_home_page,field_data_field_moel_launched WHERE field_data_field_make_home_page.entity_id = node.nid AND field_data_field_make_home_page.field_make_home_page_nid=".$_POST['makeid']." AND node.status =1 group by node.nid order by node.created desc");
				$numberofrows=@mysqli_num_rows($sql_model);
		?>
				<div class="threadsList with-thumb review-listing <?php if($numberofrows > 0){?> reviewList<?php }?>">
					<ul>
                    <?php
					...
					?>
							...
							<li>
								<a href="<?php echo $Link['field_home_page_review_url_link_value'];?>" title="<?php echo $data_model['title'];?>">
									...
									<span style="background-image: url('/sites/default/files/styles/check_mobile_thumb/public/<?php echo str_replace("public://","",$sql_img['uri']);?>'); overflow: hidden ;" class="thumb"></span>
								  <span class="text">
									<span class="ellipsis-2-line"><?php echo /*$makeRow['title']." ".*/$data_model['title'];?></span>
								  </span>
									<!--								-->
...
<!--						<a href="/forum/official-new-car-reviews/?pp=25&sort=dateline&order=desc&daysprune=-1" ripple-background="radial-gradient(red, yellow)" ripple-opacity="0.7" class="btn btn-red ripple" title="View All Car Reviews">All Reviews</a>-->
<!--					</div>-->
