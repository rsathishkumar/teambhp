...
if($_REQUEST['q']=="compare")
{
	...
	$id = @array_unique($ids);
	$s = count($id);
	$naimgpath = "<img src='/themes/mobilebhp/images/cross.png' />";
...
jQuery.getJSON("/themes/mobilebhp/includes/getCompare.php", {model: model, engine:engine, city:jQuery("#city").val()}, function(json){ 
...
jQuery("#features-entertainment tr").eq(4).find("td").eq(i).replaceWith(json.bTooth_q); jQuery("#features-entertainment tr").eq(5).find("td").eq(i).replaceWith(json.steeMoun_q); }); } function changeByVariant(id, variant, model) {var i = jQuery("#compareVariant table.compareTable tr").eq(0).find("td").index(jQuery(id).closest("td")); jQuery.getJSON("/themes/mobilebhp/includes/getCompare.php", {model: model, variant:variant, city:jQuery("#city").val()}, function(json){ 
jQuery("#other-details tr").eq(0).find("td").eq(i).html(json.price); 
