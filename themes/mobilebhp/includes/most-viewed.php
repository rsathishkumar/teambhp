...
AND node.type = 'news' and node_counter.timestamp and  node_counter.timestamp >=".strtotime("- 7 day")." and node.created >='".strtotime("- 7 days")."' and node.status=1 order by node_counter.totalcount desc limit 0,5");
/*$sql_news_weekly_viewd=@mysqli_query("SELECT node.title,node.nid,node_counter.totalcount, node_counter.daycount
FROM node, node_counter
WHERE node.nid = node_counter.nid
AND node.type = 'news' and node_counter.timestamp and  node_counter.timestamp >=".strtotime("- 7 day")." and node.status=1 order by node_counter.totalcount desc limit 0,5");*/


$sql_news_monthly_viewd=@mysqli_query("SELECT node.title,node.nid,node_counter.totalcount, node_counter.daycount
FROM node, node_counter
WHERE node.nid = node_counter.nid
AND node.type = 'news' and node_counter.timestamp and  node_counter.timestamp >=".strtotime("- 30 day")." and node.created >='".strtotime("- 30 days")."' and node.status=1 order by node_counter.totalcount desc limit 0,5");
/*$sql_news_monthly_viewd=@mysqli_query("SELECT node.title,node.nid,node_counter.totalcount, node_counter.daycount
FROM node, node_counter
WHERE node.nid = node_counter.nid
AND node.type = 'news' and node_counter.timestamp and  node_counter.timestamp >=".strtotime("- 30 day")." and node.status=1 order by node_counter.totalcount desc limit 0,5");*/
	...
	<div class="mv_tab_container">
		...
		<div id="mv_tab1" class="mv_tab_content BLR5" style="display:block">
			...
			while($weekly_news=mysqli_fetch_array($sql_news_weekly_viewd))
				{
					$weekly_news_data = node_load($weekly_news['nid']);
			...
			?>
				<li class="clearfix" rel="<?php echo url("node/".$weekly_news['nid']);?>">
					<div class="listBox clearfix">
						<div class="fleft w70 marR10" class="listBoxHolder">
							
							
							<a class="listImg" href="<?php echo url("node/".$weekly_news['nid']);?>" title="<?php echo $weekly_news_data->title;?>">
								<?php
									if($sql_img)
									{
								?>
								<!--<img src="/sites/default/files/<?php echo str_replace("public://","",$sql_img['uri']);?>" width="70" height="53" alt="<?php echo $weekly_news_data->title;?>" />-->
								<strong>
								<img src="/sites/default/files/styles/check_thumb_review_detail/public/<?php echo str_replace("public://","",$sql_img['uri']);?>" alt="<?php echo $weekly_news_data->title;?>" />
								</strong>
								<?php
									}
									else
						...
						<a href="<?php echo url("node/".$weekly_news['nid']);?>" title="<?php echo $weekly_news_data->title;?>"><?php echo $weekly_news_data->title;?></a>
...
?>
	<div id="mv_tab2" class="mv_tab_content" style="display:<?php echo $s;?>">
		<ul>
		<?php
			while($monthly_news=mysqli_fetch_array($sql_news_monthly_viewd))
			{
				$monthly_news_data = node_load($monthly_news['nid']);
		...
		?>
			...
			<li class="clearfix" rel="<?php echo url("node/".$monthly_news['nid']);?>">
				<div class="listBox clearfix">
					<div class="fleft w70 marR10">
						<a class="listImg" href="<?php echo url("node/".$monthly_news['nid']);?>" title="<?php echo $monthly_news_data->title;?>">
							<?php
								if($sql_imgmonthly)
								{
							?>
							<!--<img src="/sites/default/files/<?php echo str_replace("public://","",$sql_imgmonthly['uri']);?>" width="70" height="53" alt="<?php echo $monthly_news_data->title;?>" />-->
							<strong>
							<img src="/sites/default/files/styles/check_thumb_review_detail/public/<?php echo str_replace("public://","",$sql_imgmonthly['uri']);?>" alt="<?php echo $monthly_news_data->title;?>" />
							</strong>
							<?php
								}
								else
					...
					<a href="<?php echo url("node/".$monthly_news['nid']);?>" title="<?php echo $monthly_news_data->title;?>"><?php echo $monthly_news_data->title;?></a>
				</div><!-- list box -->
			</li>
		<?php
			}
