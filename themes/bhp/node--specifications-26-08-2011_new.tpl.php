		...
		<div class="Overview marB10">
			<div class="carOverview marB20 BLR5">
				...
				<div class="carSpeci clearfix">
					...
					<div class="specifications">									
						...
						<div class="text">
							...
							//$sql_modelengine=@mysqli_query("select field_spec_nr_engine_type_nid from field_data_field_spec_nr_engine_type where entity_id in(".substr($entyti_id,0,-1).") and bundle ='specifications'");
							 // comment as on 26-08-2011 $sql_modelengine=@mysqli_query("select node.title,field_data_field_nr_make_model.entity_id,field_data_field_nr_make_model.field_nr_make_model_nid from node,field_data_field_nr_make_model where field_data_field_nr_make_model.entity_id=node.nid and node.status=1 and field_data_field_nr_make_model.entity_id in (".substr($mid,0,-2).") group by field_data_field_nr_make_model.entity_id");
							$sql_modelengine=@mysqli_query("select nid,title from node,field_data_field_nr_make_model where field_nr_make_model_nid=".$model_id['field_nr_make_model_nid']." and field_data_field_nr_make_model.entity_id = node.nid  and bundle='engine_type'");
					...
					<div class="carWireframe">
						<img src="http://www.team-bhp.com/sites/default/files/<?php echo str_replace("public://","",$sql_line_image['uri']);?>" width="480" height="220" alt="<?php echo $data_model['title'];?>"/>
					</div><!-- w490 -->
			...
			<div class="reviewbar roundAll5 clearfix"><!-- car Review -->
				...
				<table class="speciTable">
					<thead>
						<tr>
							<th colspan="<?php echo $colspan; ?>">DIMENSIONS</th>
						</tr>	
					</thead>
					...
					<tr>
						...
						while($kwvalue=@mysqli_fetch_array($res_kw))
								...
								if($start==1)
										{
											...
											$current1 = $kwvalue['field_spec_kerb_value'];
											$start = 0;
							...
							$kVal .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$current." mm ".$prevnew."</td>";
							echo $kVal;	
							?>
					</tr>
					<tr>
					...
					<tr>
						...
						<?php
							...
							while($sc_value=@mysqli_fetch_array($res_sc))
								{
								$seat_cap_infores=@mysqli_query("select field_seating_info_value from field_data_field_seating_info where entity_id =".$sc_value['entity_id']);
							if(@mysqli_num_rows($seat_cap_infores)>0)
									{
										$Kerb_wightinfo=@mysqli_fetch_array($kerbResult);
										$prevnew = ' <a href="#" class="infoIcon">&nbsp;<div class="infoBox"><span></span>'.$Kerb_wightinfo['field_seating_info_value'].'</div></a>';
									}
								else
									{
										$prevnew = '';
								...
								if($start==1)
										{
											...
											$current1 = $sc_value['field_spec_seating_value'];
											$start = 0;
										...
										else
										{
											...
											if($prev!=$current)
												{
												$kerbResultPrev = @mysqli_query("select field_seating_info_value from field_data_field_seating_info where entity_id =".$prevID);
												if(@mysqli_num_rows($kerbResultPrev)>0)
													{
														$Kerb_wightinfo_prev=@mysqli_fetch_array($kerbResultPrev);
														$prevnewPrev = ' <a href="#" class="infoIcon">&nbsp;<div class="infoBox"><span></span>'.$Kerb_wightinfo_prev['field_seating_info_value'].'</div></a>';
													}
												else
													{
														$prevnew = '';
												...
												$cols = 0;
												$kVal .= "<td colspan=\"".$cols."\" class=\"aCenter\">";
												$kVal .= $prev." People ".$prevnewPrev."</td>";
												}
											$cols++;
										}
								}
							...
							{
								$prevnew = ' <a href="#" class="infoIcon">&nbsp;<div class="infoBox">
								<span></span>'. $Kerb_wightinfo['field_seating_info_value'].'</div>
								</a>';
							}
							else
							{
							...
							$kVal .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$current." People ".$prevnew."</td>";
							echo $kVal;	
						?>
				...
				<table class="speciTable">
					<thead>
						<tr>
							<th colspan="<?php echo $colspan; ?>">ENGINE</th>
						</tr>	
					</thead>
					<tr>
						<td>Type</td>
					...
					<tr>
						...
						/*$Cylinders=@mysqli_query($q);
								...
								if($start==1)
										...
										else
										{
											...
											if($prev!=$current)
												...
												if($cols>1)
													{
												  		$cylval .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$prev."</td>";
												  	}
												else
													{
												  		$cylval .= "<td class=\"aCenter\">".$prev."</td>";
													}
												  $cols = 0;
												}
										}
							...
							if($cols>1)
							{
								$cylval .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$current."</td>";
							}
							else
							{
								$cylval .= "<td class=\"aCenter\">".$current."</td>";
							}
							echo $cylval;
							?>
					...
					<tr>
						<td>Power to weight ratio</td>
								...
								if($start==1)
										...
										else
										{
											...
											if($prev!=$current)
												...
												if($cols>1)
													{
														$ptwrval .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$prev."</td>";
													}
												else
													{
														$ptwrval .= "<td class=\"aCenter\">".$prev."</td>";
													}
												$cols = 0;
												}
										}
							...
							if($cols>1)
							{
								$ptwrval .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$current."</td>";
							}
							else
							{
								$ptwrval .= "<td class=\"aCenter\">".$current."</td>";
							}
							echo $ptwrval;
							?>
							</tr>
					<tr>
						<td>Torque to weight ratio</td>
								...
								if($start==1)
										...
										else
										{
											...
											if($prev!=$current)
												...
												if($cols>1)
													{
														$ttwrval .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$prev."</td>";
													}
												else
													{
														$ttwrval .= "<td class=\"aCenter\">".$prev."</td>";
													}
												$cols = 0;
												}
										}
							...
							if($cols>1)
							{
								$ttwrval .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$current."</td>";
							}
							else
							{
								$ttwrval .= "<td class=\"aCenter\">".$current."</td>";
							}
						 	echo $ttwrval;
							?>
						<!--  <td>227 Nm/tonne</td>
					...
					<tr>
						<td>BHP / Liter</td>
								...
								if($start==1)
										...
										else
										{
											...
											if($prev!=$current)
												...
												if($cols>1)
													{
														$bhpltrval .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$prev."</td>";
													}
												else
													{
														$bhpltrval .= "<td class=\"aCenter\">".$prev."</td>";
													}
												$cols = 0;
												}
										}
							...
							if($cols>1)
							{
								$bhpltrval .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$current."</td>";
							}
							else
							{
								$bhpltrval .= "<td class=\"aCenter\">".$current."</td>";
							}
							echo $bhpltrval;
							?>
					</tr>
					...
					<tr>
						<td>Service Intervals</td>
								...
								if($start==1)
										...
										else
										{
											...
											if($prev!=$current)
												...
												if($cols>1)
													{
														$d_transval .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$prev."</td>";
													}
												else
													{
														$d_transval .= "<td class=\"aCenter\">".$prev."</td>";
													}
												$cols = 0;
												}
										}
							...
							if($cols>1)
							{
								$d_transval .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$current."</td>";
							}
							else
							{
								$d_transval .= "<td class=\"aCenter\">".$current."</td>";
							}
							echo $d_transval;
							?>
					</tr>
				...
				<table class="speciTable">
					<thead>
						<tr>
							<th colspan="<?php echo $colspan; ?>">SUSPENSION</th>
						</tr>	
					</thead>
					...
					<tr>
						...
						<?php
							...
							while($d_tyre_size=@mysqli_fetch_array($res_tyre_size))
								{
								$tyreResult =@mysqli_query("select field_tyre_size_caption_value from field_data_field_tyre_size_caption where entity_id =".$d_tyre_size['entity_id']);
								if(@mysqli_num_rows($tyreResult)>0)
									{
										$tyre_size_info=@mysqli_fetch_array($tyreResult);
										$prevtyre_size_info = ' <a href="#" class="infoIcon">&nbsp;<div class="infoBox"><span></span>'.$tyre_size_info['field_tyre_size_caption_value'].'</div></a>';
									}
								else
									{
										$prevtyre_size_info = '';
									...
									else
									{
										...
										if($prev!=$current)
											{
											$tyreResultPrev = @mysqli_query("select field_tyre_size_caption_value from field_data_field_tyre_size_caption where entity_id =".$prevID);
											if(@mysqli_num_rows($tyreResultPrev)>0)
												{
													$tyre_size_infoPrev=@mysqli_fetch_array($tyreResultPrev);
													$prev_seatcap_Prev = ' <a href="#" class="infoIcon">&nbsp;<div class="infoBox"><span></span>'.$tyre_size_infoPrev['field_tyre_size_caption_value'].'</div></a>';
												}
											else
												{
													$prevtyre_size_info = '';
											...
											$cols = 0;
											$tyreseizeval .= "<td colspan=\"".$cols."\" class=\"aCenter\">";
											$tyreseizeval .= $prev." ".$prev_seatcap_Prev."</td>";
											}
									}
										$cols++;
								}
							if(@mysqli_num_rows($tyreResult)>0)
							{
								$prevtyre_size_info = ' <a href="#" class="infoIcon">&nbsp;<div class="infoBox">
								<span></span>'. $tyre_size_info['field_tyre_size_caption_value'].'</div>
								</a>';
							}  
							else
							{
							...
							$tyreseizeval .= "<td colspan=\"".$cols."\" class=\"aCenter\">".$current." ".$prevtyre_size_info."</td>";
							echo $tyreseizeval;
							?>
				...
				<table class="speciTable">
					<thead>
						<tr>
							<th colspan="<?php echo $colspan + 2; ?>">FUEL EFFICIENCY</th>
						</tr>	
					</thead>
					<tr>
						<td>City</td>
				...
				if($sql_sequenceimgmainimg=='')
					...
					$sql_sequenceimgwithoutorder=@mysqli_fetch_array(mysqli_query("SELECT field_data_field_gallery_interior.delta as delta , field_gallery_interior_title as title, field_gallery_interior_fid as fidint FROM `field_data_field_gallery_interior`,field_data_field_gallery_model WHERE field_data_field_gallery_interior.entity_id=field_data_field_gallery_model.entity_id and field_data_field_gallery_model.field_gallery_model_nid='".$model_id['field_nr_make_model_nid']."' UNION SELECT field_data_field_gallery_exterior.delta as delta, field_gallery_exterior_title as title, field_gallery_exterior_fid as fidint FROM `field_data_field_gallery_exterior`, field_data_field_gallery_model WHERE field_data_field_gallery_exterior.entity_id=field_data_field_gallery_model.entity_id and field_data_field_gallery_model.field_gallery_model_nid='".$model_id['field_nr_make_model_nid']."' UNION SELECT field_data_field_gallery_engine.delta as delta, field_gallery_engine_title as title, field_gallery_engine_fid as fidint FROM `field_data_field_gallery_engine`,field_data_field_gallery_model WHERE field_data_field_gallery_engine.entity_id=field_data_field_gallery_model.entity_id and field_data_field_gallery_model.field_gallery_model_nid='".$model_id['field_nr_make_model_nid']."' UNION SELECT field_data_field_gallery_smaller.delta as delta, field_gallery_smaller_title as title, field_gallery_smaller_fid as fidint FROM `field_data_field_gallery_smaller`,field_data_field_gallery_model WHERE field_data_field_gallery_smaller.entity_id=field_data_field_gallery_model.entity_id and field_data_field_gallery_model.field_gallery_model_nid='".$model_id['field_nr_make_model_nid']."' order by delta limit 0,1"));
						if($sql_sequenceimgwithoutorder=='')
							{
							$model_modleimgname="sites/default/files/defaultmodel.jpg";
							}
						else
							{
						...
						<ul class="clearfix" id="compareUL">
							...
							<li class="clearfix">
								<div class="content firstContent clearfix" id="<?php echo $data_model['nid']; ?>"  rel="<?php echo str_replace("review/","",$sql_url['alias'])?>">
									...
									<img src="http://www.team-bhp.com/<?php echo $model_modleimgname;?>" alt="<?php echo $data_model['title'];?>" width="61" height="46" />
