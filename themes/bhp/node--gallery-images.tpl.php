...
?>
	...
	(function ($) {	
		...
		else
		{
			$.ajax({
			    	...
			    	if(l>4)
			    	{
						...
						function mycarousel_initCallback1(carousel) {
						    jQuery('.controls .next').live("click", function(){
						    	if(pos%4==0)
						    	{
						    		if(jQuery(".jcarousel-next").hasClass("jcarousel-next-disabled"))
						    		{
						    	...
						    	//carousel1.scroll(jQuery.jcarousel.intval(pos+1));
						    	}
						    });
						    jQuery('.controls .prev').live("click", function(){
						    	if(pos%4==3)
						    	{
						    		if(jQuery(".jcarousel-prev").hasClass("jcarousel-prev-disabled"))
						    		{
		 	...
		 	$.ajax({
			   type: "POST",
			   url: "/themes/bhp/show_gallerybycat.php",
			   cache: false,
			   data: "mid="+mid+"&action=show"+"&category="+val,
			   success: function(data){
				   	$('div#gallery').replaceWith(data);
				   	pos = 0;
				   	if($('div#gallery').find("span").length>1)
				   	{
				   		$('div.controls a.next').removeClass("disable");
				   	}
			   }
			 });
		 }
	...
	function explode (delimiter, string, limit) {
		...
		(function (j) {
			...
			j.ajax({
			  type: "POST",
			   url: "/themes/bhp/includes/getotherimages.php",
			   data: "mid="+mid,
			   cache: true,
			   success: function(data){
								...
								<div class="carOverview BLR5 marB10">
									<div class="clearfix">
										<div class="fleft w480">
											<h1><?php  echo /*$make_res['title']." ".*/$modeltitle['title'];?> <span>Photos</span></h1>
										</div>
									...
									<div class="clearfix photosView marT10 clearfix">
										<div class="fleft w655 photogalleryHolder clearfix">
											<?php
								 				if($num_exterior > 0 || $num_exterior1 > 0)
												{
												?>
												<div class="reviewPhoto" id="gallery">
													...
													if($num_exterior >0)
													{
														...
														?>
														<span<?php if($cnt==1) {?> class="show"<?php $showSet = 1; }?>>
															<img src="/?q=sites/default/files/styles/check_photo_gallery/public/<?php echo str_replace("public://","",str_replace("+","%2B",$data_ext_img['uri']));?>" alt="<?php echo $data_ext_img['field_gallery_exterior_title'];?>"  title="<?php echo $data_ext_img['field_gallery_exterior_title'];?>" />
														</span>
														<?php
															$cnt++;
													    }
													...
													{
														$cnt=1;
														while($data_ext_img1=mysqli_fetch_array($sql_exterior_img1))
														{
															...
															$sql_imgtitle=mysqli_fetch_array(mysqli_query("select title from node where nid=".$data_ext_img1['field_gallery_exterior_fid']));
														?>
														<span<?php if($cnt==1 && $showSet!='1') {?> class="show"<?php }?>>
															<img src="/?q=sites/default/files/styles/check_photo_gallery/public/<?php echo str_replace("public://","",$data_ext_img1['uri']);?>" alt="<?php echo $data_ext_img1['field_gallery_exterior_title'];?>"  title="<?php echo $data_ext_img1['field_gallery_exterior_title'];?>" />
														</span>
														<?php
															$cnt++;
													    }
											...
											<ul class="photoShare">
												...
												<li><a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo $fburl;?>" title="share on facebook" class="facebook" target="_blank">&nbsp;</a></li>
												<li><a href="http://twitter.com/share?text=Check out <?php echo $title." at TEAM-BHP"; ?>&url=<?php echo $twurl; ?>" target="_blank" title="share on twitter" class="twitter">&nbsp;</a></li>
											</ul>
									...
									<div class="jcarousel-skin-tango clearfix">
											...
											<a <?php if($counter==0) {?>class="active"<?php }?>onclick="return showMain(<?php echo $counter;?>);" title="<?php echo $sql_imgtitle['title'];?>" href="#">
												<span class="photosBucketImg">
													<strong>
														<img alt="<?php echo $sql_imgtitle['title'];?>" src="/?q=sites/default/files/styles/check_medium_medium/public/<?php echo str_replace("public://","",$data_ext_imgnew['uri']);?>" />
													</strong>
												</span>
											</a>
											</li>
												<?php
												 ...
												 if($num_exterior1 > 0)
												 {
													$sql_exterior_new1=@mysqli_query($q1);
													 if($num_exterior > 0)
													 {
														//$counter=1;
													 }
													...
													while($data_ext_imgnew1=mysqli_fetch_array($sql_exterior_new1))
														{
														$sql_imgtitle1=mysqli_fetch_array(mysqli_query("select title from node where nid=".$data_ext_imgnew1['field_gallery_exterior_fid']));
											?>
											<li class="jcarousel-item jcarousel-item-horizontal jcarousel-item-<?php echo $itemCount; ?> jcarousel-item-<?php echo $itemCount; ?>-horizontal">
											<a <?php if($counter==0) {?>class="active"<?php }?>onclick="return showMain(<?php echo $counter;?>);" title="<?php echo $sql_imgtitle1['title'];?>" href="#">
												<span class="photosBucketImg">
													<strong>
														<img alt="<?php echo $sql_imgtitle1['title'];?>" src="/?q=sites/default/files/styles/check_medium_medium/public/<?php echo str_replace("public://","",$data_ext_imgnew1['uri']);?>" />
													</strong>
												</span>
											</a>
											</li>
									...
									<ul class="photosBucket allPhotos" id="allPhotos">
									<?php
									if($num_exterior > 0)
									 {
										$sql_exterior_new=@mysqli_query($q);
									...
									?>
										<li>
											<a href="/?q=sites/default/files/styles/extra_large_for_review/public/<?php echo str_replace("public://","",$data_ext_imgnew['uri']);?>" title="<?php echo $sql_imgtitle['title'];?>" rel="lightbox[all]">
												...
												<span class="photosBucketImg">
													<strong>
													<img src="/?q=sites/default/files/styles/check_medium_medium/public/<?php echo str_replace("public://","",$data_ext_imgnew['uri']);?>" alt="<?php echo $sql_imgtitle['title'];?>" />
													</strong>
												</span>
									...
									?>
										<li>
											<a href="/?q=sites/default/files/styles/extra_large_for_review/public/<?php echo str_replace("public://","",$data_ext_imgnew1['uri']);?>" title="<?php echo $sql_imgtitle1['title'];?>" rel="lightbox[all]">
												<!--<img src="/sites/default/files/<?php echo str_replace("public://","",$data_ext_imgnew['uri']);?>" alt="<?php echo $sql_imgtitle['title'];?>" width="165" height="124" />-->
												<span class="photosBucketImg">
													<strong>
													<img src="/?q=sites/default/files/styles/check_medium_medium/public/<?php echo str_replace("public://","",$data_ext_imgnew1['uri']);?>" alt="<?php echo $sql_imgtitle1['title'];?>" />
													</strong>
												</span>
												...
												if($price_res['maxPrice']>=100000)
													...
													<li class="clearfix">
														<div class="content firstContent clearfix" id="<?php echo $data_model['nid']; ?>"  rel="<?php echo str_replace("review/","",$sql_url['alias'])?>">
															<div class="img">
																<strong>
																<img src="/<?php echo $model_modleimgname;?>" alt="<?php echo $data_model['title'];?>" title="<?php echo $data_model['title'];?>" />
																</strong>
															</div>
															<p class="desc">
...
<?php
drupal_add_js('themes/bhp/js/prototype.js');
drupal_add_js('themes/bhp/js/scriptaculous.js');
drupal_add_js('themes/bhp/js/lightbox.js');
?>
<link rel="stylesheet" type="text/css" href="/themes/bhp/css/lightbox-slideshow.css" media="screen" />
