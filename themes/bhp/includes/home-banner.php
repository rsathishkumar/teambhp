...
<div id="highlights" class="homeBanner">
	<div class="BLR5 bannerTabs clearfix">
		...
		$dataoffimage=@mysqli_fetch_array($res_hbanner);
			$off_thread_url = $dataoffimage['field_banner_url_url'];

			if (stristr($off_thread_url, 'http://')){

			   $off_thread_url =  str_replace('http://', 'https://', $off_thread_url);
			}
		?>
		<div id="bannerTitle" class="fright"><a href="<?php echo $off_thread_url; ?>"><?php echo $dataoffimage['title']; ?></a></div>
	</div>
			<?php
				$cnforb=1;
				$res_hbanner=@mysqli_query($sql_hpbanner);
				while($bannerdata=@mysqli_fetch_array($res_hbanner))
					...
					$thread_url = $bannerdata['field_banner_url_url'];
			...
			?>
	<div class="roundAll5 banner" title="<?php echo $bannerdata['title'];?>" rel="<?php echo $thread_url;?>" id="story-<?php echo $cnforb;?>" style="display:<?php if($cnforb==1){?>table<?php } else { ?>none<?php }?>;"><a href="<?php echo $thread_url; ?>"><img src="sites/default/files/<?php echo str_replace("public://","",$bannerdata['uri']);?>"  alt="<?php echo $bannerdata['title'];?>"/></a></div>
			<?php
					$cnforb++;
					}
			?>
