	...
	<div id="compareTitle">
		...
		<table class="speciTable compareTable tblBborder" id="car_details">
			<tr>
				<td class="blankTd">&nbsp;</td>
				<?php
$contentOpt = '<div class="carDetails search"><h3>Add Car</h3><div class="optionsBox"><div class="contentOpt" style="display:block"><select class="w160 marB15" onchange="getModelCompare(this.value);return false;" id="selMakeCompare"><option selected="selected">Select Make</option>';
...
drupal_add_js('if(screen.width<="1024") { var is1024 = 1; var totalLi = 4; } else { var is1024 = 0; var totalLi = 5; } var ownerPos; var speciPos; var featuPos; jQuery(function(){ var ownerHead = jQuery(".ownerHead"); var speciHead = jQuery(".speciHead"); var featuHead = jQuery(".featuHead"); ownerPos = ownerHead.offset(); speciPos = speciHead.offset(); featuPos = featuHead.offset(); jQuery(".infoIcon").css("zIndex","5"); jQuery(".compareHideShow .accTitle").click(function(){ if (jQuery(this).find("h4").css("position")!="fixed") { if (jQuery(this).next("div").css("display")=="none") { jQuery(this).find("h4").removeClass("ui-state-active"); jQuery(this).next("div").fadeIn(600, function(){ ownerPos = ownerHead.offset(); speciPos = speciHead.offset(); featuPos = featuHead.offset(); }); }else { jQuery(this).find("h4").addClass("ui-state-active"); jQuery(this).next("div").fadeOut(600, function(){ ownerPos = ownerHead.offset(); speciPos = speciHead.offset(); featuPos = featuHead.offset(); }); } } return false; }); jQuery(window).scroll(function() { var mainHead = jQuery("head"); var headPos = mainHead.offset(); if (headPos.top>=ownerPos.top){ jQuery(".ownerHead h4").addClass("elementHeadFix"); jQuery(".ownerHead h4").css({ position:"fixed", top:0, zIndex:10 }); jQuery(".ownerHead").next("div").css("paddingTop","48px"); }else { jQuery(".ownerHead h4").removeClass("elementHeadFix"); jQuery(".ownerHead h4").css("position","static"); jQuery(".ownerHead").next("div").css("paddingTop","0"); } if (headPos.top>=(speciPos.top)-48){ jQuery(".speciHead h4").addClass("elementHeadFix"); jQuery(".speciHead h4").css({ position:"fixed", top:48, zIndex:15 }); jQuery(".speciHead").next("div").css("paddingTop","48px"); }else { jQuery(".speciHead h4").removeClass("elementHeadFix"); jQuery(".speciHead h4").css("position","static"); jQuery(".speciHead").next("div").css("paddingTop","0"); } if (headPos.top>=(featuPos.top)-96){ jQuery(".featuHead h4").addClass("elementHeadFix"); jQuery(".featuHead h4").css({ position:"fixed", top:96, zIndex:20 }); jQuery(".featuHead").next("div").css("paddingTop","48px"); }else { jQuery(".featuHead h4").removeClass("elementHeadFix"); jQuery(".featuHead h4").css("position","static"); jQuery(".featuHead").next("div").css("paddingTop","0"); } }); if(is1024==1) { jQuery("td[colspan=\'6\']").each(function(index){ jQuery(this).attr("colspan", "5"); });jQuery("body").addClass("comparePage1024"); jQuery("table.compareTable tr").each(function(index){ jQuery(this).find("td").eq(5).remove();}); } jQuery(".infoIcon").hover(function(){ jQuery(".infoIcon").css("zIndex","5"); jQuery(this).css("zIndex","7"); }); }); function removeTD(id) {var i = jQuery("table#car_details tr:first td").index(jQuery(id).closest("td")); var j = 0; jQuery("table.compareTable tr").each(function(index){ jQuery(this).find("td").eq(i).remove(); if(jQuery(this).find("td").length>0) { if(jQuery("div.search").length==0) { if(j==0) { jQuery(this).append(\'<td>'.$contentOpt.'</td>\');} else {jQuery(this).append("<td></td>");} } else {jQuery(this).append("<td></td>");} } j++; }); } function changeByEngine(id, engine, model) {var i = jQuery("table.compareTable tr").eq(1).find("td").index(jQuery(id).closest("td")); jQuery.getJSON("/themes/bhp/includes/getCompare.php", {model: model, engine:engine, city:jQuery("#city").val()}, function(json){ jQuery("table.compareTable tr").eq(2).find("td").eq(i).html(json.variant); jQuery("table.compareTable tr").eq(3).find("td").eq(i).html(json.price); jQuery("table.compareTable tr").eq(14).find("td").eq(i).html(json.length); jQuery("table.compareTable tr").eq(15).find("td").eq(i).html(json.wheel); jQuery("table.compareTable tr").eq(16).find("td").eq(i).html(json.track); jQuery("table.compareTable tr").eq(17).find("td").eq(i).html(json.kerb); jQuery("table.compareTable tr").eq(18).find("td").eq(i).html(json.ground); jQuery("table.compareTable tr").eq(19).find("td").eq(i).html(json.radius); jQuery("table.compareTable tr").eq(20).find("td").eq(i).html(json.seatCapacity); jQuery("table.compareTable tr").eq(21).find("td").eq(i).html(json.bootCapacity);  jQuery("table.compareTable tr").eq(22).find("td").eq(i).html(json.fuelCapacity); jQuery("table.compareTable tr").eq(24).find("td").eq(i).html(json.eType); jQuery("table.compareTable tr").eq(25).find("td").eq(i).html(json.displacement); jQuery("table.compareTable tr").eq(26).find("td").eq(i).html(json.cylinders); jQuery("table.compareTable tr").eq(27).find("td").eq(i).html(json.valveTrain); jQuery("table.compareTable tr").eq(28).find("td").eq(i).html(json.boreStroke); jQuery("table.compareTable tr").eq(29).find("td").eq(i).html(json.compressionRatio); jQuery("table.compareTable tr").eq(30).find("td").eq(i).html(json.maxPower); jQuery("table.compareTable tr").eq(31).find("td").eq(i).html(json.maxTorque); jQuery("table.compareTable tr").eq(32).find("td").eq(i).html(json.powerWeight); jQuery("table.compareTable tr").eq(33).find("td").eq(i).html(json.torqueWeight); jQuery("table.compareTable tr").eq(34).find("td").eq(i).html(json.bhpLiter); jQuery("table.compareTable tr").eq(35).find("td").eq(i).html(json.driveTrain); jQuery("table.compareTable tr").eq(36).find("td").eq(i).html(json.eTransmission); jQuery("table.compareTable tr").eq(37).find("td").eq(i).html(json.serviceInterval); jQuery("table.compareTable tr").eq(39).find("td").eq(i).html(json.steering); jQuery("table.compareTable tr").eq(40).find("td").eq(i).html(json.fSuspension); jQuery("table.compareTable tr").eq(41).find("td").eq(i).html(json.rSuspension); jQuery("table.compareTable tr").eq(42).find("td").eq(i).html(json.tyreSize); jQuery("table.compareTable tr").eq(43).find("td").eq(i).html(json.brakes); jQuery("table.compareTable tr").eq(45).find("td").eq(i).replaceWith(json.air_q); jQuery("table.compareTable tr").eq(46).find("td").eq(i).replaceWith(json.abs_q); jQuery("table.compareTable tr").eq(47).find("td").eq(i).replaceWith(json.traction_q); jQuery("table.compareTable tr").eq(48).find("td").eq(i).replaceWith(json.esc_q); jQuery("table.compareTable tr").eq(49).find("td").eq(i).replaceWith(json.fog_q); jQuery("table.compareTable tr").eq(50).find("td").eq(i).replaceWith(json.rear_q); jQuery("table.compareTable tr").eq(51).find("td").eq(i).replaceWith(json.engine_q); jQuery("table.compareTable tr").eq(52).find("td").eq(i).replaceWith(json.alloy_q); jQuery("table.compareTable tr").eq(54).find("td").eq(i).replaceWith(json.psteer_q); jQuery("table.compareTable tr").eq(55).find("td").eq(i).replaceWith(json.tilt_q); jQuery("table.compareTable tr").eq(56).find("td").eq(i).replaceWith(json.st_reach_q); jQuery("table.compareTable tr").eq(57).find("td").eq(i).replaceWith(json.fHeight_q); jQuery("table.compareTable tr").eq(58).find("td").eq(i).replaceWith(json.lumbar_q); jQuery("table.compareTable tr").eq(59).find("td").eq(i).replaceWith(json.dead_q); jQuery("table.compareTable tr").eq(60).find("td").eq(i).replaceWith(json.arm_q); jQuery("table.compareTable tr").eq(61).find("td").eq(i).replaceWith(json.mirror_q); jQuery("table.compareTable tr").eq(62).find("td").eq(i).replaceWith(json.infoDisplay_q); jQuery("table.compareTable tr").eq(63).find("td").eq(i).replaceWith(json.sensors_q); jQuery("table.compareTable tr").eq(64).find("td").eq(i).replaceWith(json.gear_q); jQuery("table.compareTable tr").eq(66).find("td").eq(i).replaceWith(json.remote_q); jQuery("table.compareTable tr").eq(67).find("td").eq(i).replaceWith(json.central_q); jQuery("table.compareTable tr").eq(68).find("td").eq(i).replaceWith(json.window_q); jQuery("table.compareTable tr").eq(69).find("td").eq(i).replaceWith(json.airC_q); jQuery("table.compareTable tr").eq(70).find("td").eq(i).replaceWith(json.climate_q); jQuery("table.compareTable tr").eq(71).find("td").eq(i).replaceWith(json.vents_q); jQuery("table.compareTable tr").eq(72).find("td").eq(i).replaceWith(json.seats_q); jQuery("table.compareTable tr").eq(73).find("td").eq(i).replaceWith(json.sunRoof_q); jQuery("table.compareTable tr").eq(74).find("td").eq(i).replaceWith(json.fold_q); jQuery("table.compareTable tr").eq(75).find("td").eq(i).replaceWith(json.split_q); jQuery("table.compareTable tr").eq(77).find("td").eq(i).replaceWith(json.speakers_q); jQuery("table.compareTable tr").eq(78).find("td").eq(i).replaceWith(json.cd_q); jQuery("table.compareTable tr").eq(79).find("td").eq(i).replaceWith(json.aux_q); jQuery("table.compareTable tr").eq(80).find("td").eq(i).replaceWith(json.usb_q); jQuery("table.compareTable tr").eq(81).find("td").eq(i).replaceWith(json.bTooth_q); jQuery("table.compareTable tr").eq(82).find("td").eq(i).replaceWith(json.steeMoun_q); }); } function changeByVariant(id, variant, model) {var i = jQuery("table.compareTable tr").eq(2).find("td").index(jQuery(id).closest("td")); jQuery.getJSON("/themes/bhp/includes/getCompare.php", {model: model, variant:variant, city:jQuery("#city").val()}, function(json){ jQuery("table.compareTable tr").eq(3).find("td").eq(i).html(json.price); jQuery("table.compareTable tr").eq(45).find("td").eq(i).replaceWith(json.air_q); jQuery("table.compareTable tr").eq(46).find("td").eq(i).replaceWith(json.abs_q); jQuery("table.compareTable tr").eq(47).find("td").eq(i).replaceWith(json.traction_q); jQuery("table.compareTable tr").eq(48).find("td").eq(i).replaceWith(json.esc_q); jQuery("table.compareTable tr").eq(49).find("td").eq(i).replaceWith(json.fog_q); jQuery("table.compareTable tr").eq(50).find("td").eq(i).replaceWith(json.rear_q); jQuery("table.compareTable tr").eq(51).find("td").eq(i).replaceWith(json.engine_q); jQuery("table.compareTable tr").eq(52).find("td").eq(i).replaceWith(json.alloy_q); jQuery("table.compareTable tr").eq(54).find("td").eq(i).replaceWith(json.psteer_q); jQuery("table.compareTable tr").eq(55).find("td").eq(i).replaceWith(json.tilt_q); jQuery("table.compareTable tr").eq(56).find("td").eq(i).replaceWith(json.st_reach_q); jQuery("table.compareTable tr").eq(57).find("td").eq(i).replaceWith(json.fHeight_q); jQuery("table.compareTable tr").eq(58).find("td").eq(i).replaceWith(json.lumbar_q); jQuery("table.compareTable tr").eq(59).find("td").eq(i).replaceWith(json.dead_q); jQuery("table.compareTable tr").eq(60).find("td").eq(i).replaceWith(json.arm_q); jQuery("table.compareTable tr").eq(61).find("td").eq(i).replaceWith(json.mirror_q); jQuery("table.compareTable tr").eq(62).find("td").eq(i).replaceWith(json.infoDisplay_q); jQuery("table.compareTable tr").eq(63).find("td").eq(i).replaceWith(json.sensors_q); jQuery("table.compareTable tr").eq(64).find("td").eq(i).replaceWith(json.gear_q); jQuery("table.compareTable tr").eq(66).find("td").eq(i).replaceWith(json.remote_q); jQuery("table.compareTable tr").eq(67).find("td").eq(i).replaceWith(json.central_q); jQuery("table.compareTable tr").eq(68).find("td").eq(i).replaceWith(json.window_q); jQuery("table.compareTable tr").eq(69).find("td").eq(i).replaceWith(json.airC_q); jQuery("table.compareTable tr").eq(70).find("td").eq(i).replaceWith(json.climate_q); jQuery("table.compareTable tr").eq(71).find("td").eq(i).replaceWith(json.vents_q); jQuery("table.compareTable tr").eq(72).find("td").eq(i).replaceWith(json.seats_q); jQuery("table.compareTable tr").eq(73).find("td").eq(i).replaceWith(json.sunRoof_q); jQuery("table.compareTable tr").eq(74).find("td").eq(i).replaceWith(json.fold_q); jQuery("table.compareTable tr").eq(75).find("td").eq(i).replaceWith(json.split_q); jQuery("table.compareTable tr").eq(77).find("td").eq(i).replaceWith(json.speakers_q); jQuery("table.compareTable tr").eq(78).find("td").eq(i).replaceWith(json.cd_q); jQuery("table.compareTable tr").eq(79).find("td").eq(i).replaceWith(json.aux_q); jQuery("table.compareTable tr").eq(80).find("td").eq(i).replaceWith(json.usb_q); jQuery("table.compareTable tr").eq(81).find("td").eq(i).replaceWith(json.bTooth_q); jQuery("table.compareTable tr").eq(82).find("td").eq(i).replaceWith(json.steeMoun_q); var total = jQuery(".nid").length; if(json.Safety!="") {jQuery.each(json.Safety, function(k, object) { jQuery.each(object, function(property, value) { var trID = (property.replace("_", "")).toLowerCase(); if(jQuery("tr#"+trID).length>0) { jQuery("tr#"+trID).find("td").eq(i).replaceWith(value); } else { jQuery("table#features-safety").append("<tr id=\'" + trID + "\'><td>" + property.replace(\'_\', \' \') + "</td></tr>"); for(var trCount=0; trCount<jQuery("table.compareTable tr").eq(75).find("td").length-1; trCount++){ if(trCount<total){ jQuery("tr#"+trID).append("<td class=\'aCenter\'>N/A</td>");} else { jQuery("tr#"+trID).append("<td>&nbsp;</td>"); } } jQuery("tr#"+trID).find("td").eq(i).replaceWith(value); } }); });} }); } function setID(model) {var id = model.options[model.selectedIndex].getAttribute("rel"); if(jQuery("input[value=\'"+id+"\']").length>0) {jQuery("div.search h3").text("Add other car").addClass("error"); jQuery("a#addCar").attr("onClick", "return false;"); } else {jQuery("div.search h3").text("Add Car").removeClass("error"); jQuery("#addCar").attr("onClick", "getCar(\'"+id+"\'); return false;"); } } function getModelCompare(id){ jQuery("select#selModelCompare").attr("disabled", "disabled"); jQuery.ajax({type: "POST",url: "/themes/bhp/includes/getModel.php",data: "make="+id,success: function(data){ jQuery("select#selModelCompare").html(data); jQuery("select#selModelCompare").removeAttr("disabled") }}); } function getCar(mod){ jQuery.getJSON("/themes/bhp/includes/getCompare.php", {model: mod, city:jQuery("#city").val()}, function(json) { var i = jQuery("table#car_details tr:first td").index(jQuery("div.search").closest("td")); jQuery("#car_title tr:first").find("td").eq(i).html("<div class=\"carDetails\"><h3>"+json.title+"</h3></div>"); jQuery("div.search").html("</h3><img alt=\""+json.title+"\" src=\"http://www.team-bhp.com/"+json.uri+"\"><div class=\"clearfix\"><a href=\"/"+json.url+"\" class=\"fleft reviewBtn\"><span>Full Review</span></a><a href=\"#\" onclick=\"removeTD(this); return false;\" class=\"fleft removeBtn\" rel=\""+json.nid+"\">&nbsp;</a></div><input type=\"hidden\" id=\"nid"+(i-1)+"\" name=\"nid"+(i-1)+"\" class=\"nid\" value=\""+json.nid+"\" />").removeClass("search");  jQuery("div#compareEngine").find("td").eq(i).html(json.engine);  jQuery("div#compareVariant").find("td").eq(i).html(json.variant); jQuery("table.compareTable tr").eq(4).find("td").eq(i).html(json.price); jQuery("table.compareTable tr").eq(5).find("td").eq(i).html(json.fuel); jQuery("table.compareTable tr").eq(6).find("td").eq(i).html(json.liked); jQuery("table.compareTable tr").eq(7).find("td").eq(i).html(json.dislike); jQuery("table.compareTable tr").eq(8).find("td").eq(i).html(json.upkeep); jQuery("table.compareTable tr").eq(9).find("td").eq(i).html(json.reliability); jQuery("table.compareTable tr").eq(10).find("td").eq(i).html(json.ser); jQuery("table.compareTable tr").eq(11).find("td").eq(i).html(json.sWar); jQuery("table.compareTable tr").eq(12).find("td").eq(i).html(json.eWar); jQuery("table.compareTable tr").eq(13).find("td").eq(i).html(json.resale); jQuery("table.compareTable tr").eq(15).find("td").eq(i).html(json.length); jQuery("table.compareTable tr").eq(16).find("td").eq(i).html(json.wheel); jQuery("table.compareTable tr").eq(17).find("td").eq(i).html(json.track); jQuery("table.compareTable tr").eq(18).find("td").eq(i).html(json.kerb); jQuery("table.compareTable tr").eq(19).find("td").eq(i).html(json.ground); jQuery("table.compareTable tr").eq(20).find("td").eq(i).html(json.radius); jQuery("table.compareTable tr").eq(21).find("td").eq(i).html(json.seatCapacity); jQuery("table.compareTable tr").eq(22).find("td").eq(i).html(json.bootCapacity); jQuery("table.compareTable tr").eq(23).find("td").eq(i).html(json.fuelCapacity); jQuery("table.compareTable tr").eq(25).find("td").eq(i).html(json.eType); jQuery("table.compareTable tr").eq(26).find("td").eq(i).html(json.displacement); jQuery("table.compareTable tr").eq(27).find("td").eq(i).html(json.cylinders); jQuery("table.compareTable tr").eq(28).find("td").eq(i).html(json.valveTrain); jQuery("table.compareTable tr").eq(29).find("td").eq(i).html(json.boreStroke); jQuery("table.compareTable tr").eq(30).find("td").eq(i).html(json.compressionRatio); jQuery("table.compareTable tr").eq(31).find("td").eq(i).html(json.maxPower); jQuery("table.compareTable tr").eq(32).find("td").eq(i).html(json.maxTorque); jQuery("table.compareTable tr").eq(33).find("td").eq(i).html(json.powerWeight); jQuery("table.compareTable tr").eq(34).find("td").eq(i).html(json.torqueWeight); jQuery("table.compareTable tr").eq(35).find("td").eq(i).html(json.bhpLiter); jQuery("table.compareTable tr").eq(36).find("td").eq(i).html(json.driveTrain); jQuery("table.compareTable tr").eq(37).find("td").eq(i).html(json.eTransmission); jQuery("table.compareTable tr").eq(38).find("td").eq(i).html(json.serviceInterval); jQuery("table.compareTable tr").eq(40).find("td").eq(i).html(json.steering); jQuery("table.compareTable tr").eq(41).find("td").eq(i).html(json.fSuspension); jQuery("table.compareTable tr").eq(42).find("td").eq(i).html(json.rSuspension); jQuery("table.compareTable tr").eq(43).find("td").eq(i).html(json.tyreSize); jQuery("table.compareTable tr").eq(44).find("td").eq(i).html(json.brakes); jQuery("table.compareTable tr").eq(46).find("td").eq(i).replaceWith(json.air_q); jQuery("table.compareTable tr").eq(47).find("td").eq(i).replaceWith(json.abs_q); jQuery("table.compareTable tr").eq(48).find("td").eq(i).replaceWith(json.traction_q); jQuery("table.compareTable tr").eq(49).find("td").eq(i).replaceWith(json.esc_q); jQuery("table.compareTable tr").eq(50).find("td").eq(i).replaceWith(json.fog_q); jQuery("table.compareTable tr").eq(51).find("td").eq(i).replaceWith(json.rear_q); jQuery("table.compareTable tr").eq(52).find("td").eq(i).replaceWith(json.engine_q); jQuery("table.compareTable tr").eq(53).find("td").eq(i).replaceWith(json.alloy_q); jQuery("table.compareTable tr").eq(55).find("td").eq(i).replaceWith(json.psteer_q); jQuery("table.compareTable tr").eq(56).find("td").eq(i).replaceWith(json.tilt_q); jQuery("table.compareTable tr").eq(57).find("td").eq(i).replaceWith(json.st_reach_q); jQuery("table.compareTable tr").eq(58).find("td").eq(i).replaceWith(json.fHeight_q); jQuery("table.compareTable tr").eq(59).find("td").eq(i).replaceWith(json.lumbar_q); jQuery("table.compareTable tr").eq(60).find("td").eq(i).replaceWith(json.dead_q); jQuery("table.compareTable tr").eq(61).find("td").eq(i).replaceWith(json.arm_q); jQuery("table.compareTable tr").eq(62).find("td").eq(i).replaceWith(json.mirror_q); jQuery("table.compareTable tr").eq(63).find("td").eq(i).replaceWith(json.infoDisplay_q); jQuery("table.compareTable tr").eq(64).find("td").eq(i).replaceWith(json.sensors_q); jQuery("table.compareTable tr").eq(65).find("td").eq(i).replaceWith(json.gear_q); jQuery("table.compareTable tr").eq(67).find("td").eq(i).replaceWith(json.remote_q); jQuery("table.compareTable tr").eq(68).find("td").eq(i).replaceWith(json.central_q); jQuery("table.compareTable tr").eq(69).find("td").eq(i).replaceWith(json.window_q); jQuery("table.compareTable tr").eq(70).find("td").eq(i).replaceWith(json.airC_q); jQuery("table.compareTable tr").eq(71).find("td").eq(i).replaceWith(json.climate_q); jQuery("table.compareTable tr").eq(72).find("td").eq(i).replaceWith(json.vents_q); jQuery("table.compareTable tr").eq(73).find("td").eq(i).replaceWith(json.seats_q); jQuery("table.compareTable tr").eq(74).find("td").eq(i).replaceWith(json.sunRoof_q); jQuery("table.compareTable tr").eq(75).find("td").eq(i).replaceWith(json.fold_q); jQuery("table.compareTable tr").eq(76).find("td").eq(i).replaceWith(json.split_q); jQuery("table.compareTable tr").eq(78).find("td").eq(i).replaceWith(json.speakers_q); jQuery("table.compareTable tr").eq(79).find("td").eq(i).replaceWith(json.cd_q); jQuery("table.compareTable tr").eq(80).find("td").eq(i).replaceWith(json.aux_q); jQuery("table.compareTable tr").eq(81).find("td").eq(i).replaceWith(json.usb_q); jQuery("table.compareTable tr").eq(82).find("td").eq(i).replaceWith(json.bTooth_q); jQuery("table.compareTable tr").eq(83).find("td").eq(i).replaceWith(json.steeMoun_q); if(i<5) {jQuery("table#car_details tr:first td").eq(i+1).html(\''.$contentOpt.'\');} } ); } function removeCar(id) { jQuery(id).closest("li.clearfix").remove(); jQuery("li#compareLi").before("<li class=\'num\'><span class=\'n\'>5</span></li>"); if(jQuery("ul#compareUL").find("select#selMakeCompare").length==0) { jQuery("ul#compareUL li.num").html(\''.$contentOpt.'\'); jQuery("ul#compareUL li.num").attr("class", "clearfix"); } else { jQuery("ul#compareUL li").each(function(index){ if(jQuery(this).hasClass("num")) { jQuery(this).find("span.n").text(index+1); } }); } } function addCommas(nStr) { nStr += ""; x = nStr.split("."); x1 = x[0]; x2 = x.length > 1 ? "." + x[1] : ""; var rgx = /(\d+)(\d{3})/; while (rgx.test(x1)) { x1 = x1.replace(rgx, "$1" + "," + "$2"); } return x1 + x2; } function changePrice(city) { var nid=""; jQuery("#city").val(city); jQuery("select.w110").each(function(index){ nid = jQuery(this).val(); jQuery.ajax({type: "POST", url: "/themes/bhp/includes/getPrice.php", data: "city="+city+"&nid="+nid, success: function(data){if(data!="") { jQuery(".compareTable tr").eq(3).find("td").eq(index+1).html("<span class=\"WebRupee\">Rs.</span>"+addCommas(data)); } else { jQuery(".compareTable tr").eq(3).find("td").eq(index+1).html("No price available"); } }}); })} ', 'inline');
				...
				for($i=0; $i<5; $i++)
				{
					...
					<td>
						<div class="carDetails">
							<!-- <h3><?php echo $data_model['title'];?></h3> -->
							<!--<img src="http://www.team-bhp.com/sites/default/files/<?php echo str_replace("public://","",$data_model['uri']);?>" width="165" height="124" alt="<?php echo $data_model['title'];?>" />-->
			...
			<table class="speciTable compareTable" >
			<tr>
				<td>On-road Price
