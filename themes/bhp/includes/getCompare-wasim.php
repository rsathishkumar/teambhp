...
if(@mysqli_num_rows($sql_sfty)>0)
	...
	print_r($datearr);
$sql_d_enhance=@mysqli_query("select feature_option from team_bhp_features where nid=".$fid." and category='Driver Enhancements'");
...
$sql_Convenience=@mysqli_query("select feature_option from team_bhp_features where nid=".$fid." and category='Convenience'");
...
$sql_ent=@mysqli_query("select feature_option from team_bhp_features where nid=".$fid." and category='Entertainment'");
...
if(count($actual_sfty_ar)>0)
	{
	print_r($actual_sfty_ar);
$data=array_push($data,$actual_sfty_ar);
	}
if(count($actual_enhance_ar)>0)
	{
$data=array_push($data,$actual_enhance_ar);
	}
if(count($actual_Convenience_ar)>0)
	{
$data=array_push($data,$actual_Convenience_ar);
	}
if(count($actual_ent_ar)>0)
	{
$data=array_push($data,$actual_ent_ar);
	}
