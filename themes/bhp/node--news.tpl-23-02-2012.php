		...
		<div class="article">
								...
								if(sizeof($node->field_news_images['und'])>0)
									...
									?>
										<a href="http://www.team-bhp.com/?q=sites/default/files/styles/check_extra_large_for_review/public/<?php echo str_replace("public://","",$data_news['uri']);?>" class="lightbox mainPhoto<?php  if($cnt_newsimg==0) { ?> active<?php } ?>"<?php if($cnt_newsimg==0) {?> style="display:table"<?php } else {?> style="display:none"<?php }?> title="<?php echo $node->title;?>" rel="lightbox[news]">
											...
											<strong>
												<img src="http://www.team-bhp.com/?q=sites/default/files/styles/check_large_review/public/<?php echo str_replace("public://","",$data_news['uri']);?>"<?php if($cnt_newsimg==0) {?> id="mainPhoto"<?php } ?> alt="<?php echo $node->title;?> " />
											</strong>
										</a>
			...
			<div class="clearfix articleNavi">
				...
				<?php
					...
					{
					$res1 = mysqli_fetch_array(mysqli_query("select nid from node where type='news' and status= 1 and nid<".$node->nid." limit 1"));
					$res_url = mysqli_query("select alias from url_alias where source='node/".$res1['nid']."'");
					}
