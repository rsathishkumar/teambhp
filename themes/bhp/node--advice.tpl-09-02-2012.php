												...
												if(count($descArray)>1)
												{
													...
													for($i=0; $i<count($descArray); $i++)
														...
														<?php
															...
															$node->field_advice_content['und'][0]['safe_value'][$i]=str_replace("><p></p>","",$node->field_advice_content['und'][0]['safe_value'][$i]);
															$node->field_advice_content['und'][0]['safe_value'][$i]=str_replace('!--start--',"<strong class='reviewQuotes'><span class='openingQuotes'>&ldquo;</span>",$node->field_advice_content['und'][0]['safe_value'][$i]);
															$node->field_advice_content['und'][0]['safe_value'][$i]=str_replace('!--end--',"<span class='closingQuots'>&rdquo;</span></strong>",$node->field_advice_content['und'][0]['safe_value'][$i]);*/
															if($i!=0)
																{
																echo "<p>";
																}
															// $str_toreplace=str_replace("&gt;","",$descArray[$i]);
																//echo str_replace($search, $replace, $descArray[$i]);
																/*$position='';
																$strtposition='';
																$lengthminus5=strlen($node->field_advice_content['und'][0]['safe_value'][$i])-5;
																$strtposition=strpos(substr(trim($node->field_advice_content['und'][0]['safe_value'][$i]),0,1),"&gt;");
																 $position=strpos(substr(trim($node->field_advice_content['und'][0]['safe_value'][$i]),$lengthminus5,strlen($node->field_advice_content['und'][0]['safe_value'][$i])),'<');
																	if($position>=0)
																		{
																		//strlen($node->field_advice_content['und'][0]['safe_value'][$i])-$lengthminus5;
																		//str_replace("<","",substr($node->field_advice_content['und'][0]['safe_value'][$i],strlen($node->field_advice_content['und'][0]['safe_value'][$i])-$position,strlen($node->field_advice_content['und'][0]['safe_value'][$i])));
																//$node->field_advice_content['und'][0]['safe_value'][$i]=str_replace("<","",substr($node->field_advice_content['und'][0]['safe_value'][$i],strlen($node->field_advice_content['und'][0]['safe_value'][$i])-5,strlen($node->field_advice_content['und'][0]['safe_value'][$i])));		
																		}
																		if($i==0)
																			{
																			if($position>=0)
																				{
																	$node->field_advice_content['und'][0]['safe_value'][$i]=substr($node->field_advice_content['und'][0]['safe_value'][$i],0,strlen($node->field_advice_content['und'][0]['safe_value'][$i])-1);
																				}
																	print $node->field_advice_content['und'][0]['safe_value'][$i];
																			...
																			if($strtposition!='')
																				{
																				//$node->field_advice_content['und'][0]['safe_value'][$i]=substr($node->field_advice_content['und'][0]['safe_value'][$i],0,strlen($node->field_advice_content['und'][0]['safe_value'][$i]));
																				}
																			if($position>=0)
																				{
																			$node->field_advice_content['und'][0]['safe_value'][$i]=substr($node->field_advice_content['und'][0]['safe_value'][$i],0,strlen($node->field_advice_content['und'][0]['safe_value'][$i])-1);
																	...
																	print $node->field_advice_content['und'][0]['safe_value'][$i];		
																			...
																			print trim($node->field_advice_content['und'][0]['safe_value'][$i]);
