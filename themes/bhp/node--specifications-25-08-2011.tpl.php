		...
		<div class="Overview marB10">
			...
			<div class="reviewbar roundAll5 clearfix"><!-- car Review -->
				...
				<table class="speciTable">
					...
					<tr>
						...
						<td  class="aCenter"<?php  if($nolwh==1){?> colspan="<?php echo $nofmodel?>"<?php } else if($ct>1 && $ct==$nolwh && $nolwh<4) { ?> colspan="<?php echo $cval;?>" <?php }?> ><?php echo $dimensionvalvalue['field_spec_length_value']." mm";?></td>
							...
							$res_lwh=mysqli_query("select DISTINCT(field_spec_length_value) from field_data_field_spec_length  where entity_id in (".substr($entyti_id,0,-1).") order by entity_id");
							$start = "1";
							$cols = "1";
							while($dimensionvalvalue=mysqli_fetch_array($res_lwh))
								{
						...
						<?php
							...
							while($wbvalue=@mysqli_fetch_array($res_wb))
								...
								if($start==1)
										...
										{
											$prev = $current;
											$current_wheel = $wbvalue['field_spec_wheel_value'];
											if($prev!=$current_wheel)
												{
												   $current_wheel=$wbvalue['field_spec_wheel_value'];
												  $prev=$current_wheel;
					...
					<tr>
						<td>Track : Front / Rear</td>
							...
							while($frvalue=@mysqli_fetch_array($res_tfr))
								...
								if($start==1)
										{
										echo $current = $frvalue['field_spec_track_value'];
										$start = 0;
										}
										else
										{
											$prev = $current;
											echo $current_Front = $frvalue['field_spec_track_value'];
											if($prev!=$current_wheel)
												{
												   $current_Front=$frvalue['field_spec_track_value'];
												   $prev=$current_Front;
				...
				<table class="speciTable">
					<thead>
						<tr>
							<th colspan="<?php echo $colspan; ?>">FUEL EFFICIENCY</th>
						</tr>	
					</thead>
					<tr>
						<td>City</td>
						...
						<ul class="clearfix" id="compareUL">
							...
							<li class="clearfix">
								<div class="content firstContent clearfix" id="<?php echo $data_model['nid']; ?>">
									...
									<img src="http://www.team-bhp.com/?q=sites/default/files/styles/thumb_review_detail/public/<?php echo str_replace("public://","",$data_model['uri']);?>" alt="<?php echo $data_model['title'];?>" width="61" height="46" />
